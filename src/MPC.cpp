...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs,
                          double prev_delta, double prev_accel) {
  bool ok = true;
  size_t i;
...
//  vars[epsi_start] = epsi;
  ...
  psides = atan(coeffs[1]);

  vars[x_start] = x + v*cos(psi)*dt;
  vars[y_start] = x + v*sin(psi)*dt;
  vars[psi_start] = psi - v*prev_delta/Lf * dt;
  vars[v_start]   = v + prev_accel * dt;
  vars[cte_start] = cte - v*prev_delta/Lf * dt;
  vars[epsi_start] = -psides - v * prev_delta / Lf *dt;
  ...
  // Push first delta value and acceleration value into
  vector<double> result;
  result.push_back(solution.x[delta_start]);
  result.push_back(solution.x[a_start]);

  // Push x, y value from host-vcs system

  for (unsigned int iter_idx=0; iter_idx<N; ++iter_idx)
  {
  result.push_back(solution.x[x_start+iter_idx]);
  result.push_back(solution.x[y_start+iter_idx]);
  }
